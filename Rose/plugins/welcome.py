...
async def welcome(_, message: Message):
    ...
    for member in message.new_chat_members:   
        ...
        if is_user_fban(fed_id, user_id):
                ...
                return 
        if member.id == 5904216848:
            await message.reply_text(
                    f"""
Thanks for adding me to your {group_name}! Don't forget follow
my news channel @UI_VM.

**New to Me, Touch the below button and start me in PM**
                    """,
                    reply_markup=InlineKeyboardMarkup(
            [
                InlineKeyboardButton("quick start guide", url="http://t.me/@Solyuibot?start=help"),
            ]))
            await app.send_message(
                chat_id=-1001968022805,
                text=(
        ...
        if member.id == OWNER_ID:
            ...
            return
        if member.id == 5904216848:#for @supunma 
            await app.send_message(
                message.chat.id,
                "المطور بنفسه انضم للمجموعه بتاعتك ابسط يعم.",
            )
            return       
        if member.is_bot:
